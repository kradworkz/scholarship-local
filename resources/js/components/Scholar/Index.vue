            ...
            <div class="card-body" :style="{ height: height + 'px' }">
                <div class="row mb-3">
                    <div class="col-xl-6 col-sm-6 form-inline">
                        <router-link :to="{ name: 'qualifier/sync' }"> <button type="button" class="btn btn-danger waves-effect waves-light mr-2"><i class='bx bx-plus'></i></button></router-link>
                ...
                <div class="table-responsive">
                    <table class="table table-centered table-nowrap">
                        ...
                        <tbody>
                            <tr v-for="user in users" v-bind:key="user.id" style="cursor: pointer;">
                                <td>
                                ...
                                <td style="cursor: pointer;" @click="profile(user)">
                                    ...
                                    <p class="font-size-11 text-muted mb-0">{{user.barangay}}, {{user.municipality.name }}, {{user.province.name }}</p>
                                </td>
                                ...
                                <td class="text-center">
                                    <h5 class="font-size-11 mb-0 text-dark">{{user.school.school.name }} - {{user.school.campus }}</h5>
                                    <p class="font-size-11 text-muted mb-0">{{user.course.name }}</p>
                                </td>
                                <td class="text-center font-weight-bold text-primary">{{user.awarded_year}}</td>
...
export default {
    ...
    methods: {
        ...
        fetch(page_url) {
            ...
            page_url = page_url || this.currentUrl + '/request/scholars/'+s+'/'+c+'/'+this.counts+'/'+y+'/'+key;
        ...
        filter(data,type){
            ...
            let vm = this; let key;
            (this.keyword != '' && this.keyword != null) ? key = this.keyword : key = '-';
            this.fetch(this.currentUrl + '/request/scholars/'+this.status+'/'+this.category+'/'+this.counts+'/'+this.year+'/'+key);
        },
